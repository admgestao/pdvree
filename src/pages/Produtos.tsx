...
export default function Produtos() {
  ...
  function abrirModalLote() {
    ...
    setShowForm(false); // Fecha o modal principal ao fundo
  ...
  async function verificarCodigo(codigoStr: string) {
    ...
    if (prodIdEncontrado) {
        ...
        if (fullProd) {
            // Filtra para exibir APENAS os lotes que possuem o código exato em conflito
            const lotesFiltrados = (fullLotes || []).filter(l => l.codigo_barras === codigoStr);
  ...
  async function handleSaveLote() {
    ...
    if (loteFormMode === 'edit' && loteEditId) {
        ...
        if (oldLote) {
            ...
            // Busca os lotes atualizados para recalcular custo médio geral e identificar lote mais recente
            const { data: updatedLotes } = await supabase.from('produto_lotes').select('*').eq('produto_id', editing.id);

            let calcCustoTotal = 0;
            let calcEstoqueTotal = 0;
            let loteMaisRecente = null;

            if (updatedLotes && updatedLotes.length > 0) {
                updatedLotes.forEach(l => {
                    const q = Number(l.quantidade_atual || l.quantidade);
                    if (q > 0) {
                        calcCustoTotal += q * Number(l.custo);
                        calcEstoqueTotal += q;
                    }
                });

                loteMaisRecente = updatedLotes.sort((a, b) => {
                    const dateA = new Date(a.data_entrada || '2000-01-01').getTime();
                    const dateB = new Date(b.data_entrada || '2000-01-01').getTime();
                    if (dateA === dateB) {
                        const crA = new Date(a.criado_em || '2000-01-01').getTime();
            ...
            // Adiciona o estoque base (caso exista algo fora de lote) para o custo médio
            const lotesQtdOriginal = editing.lotes?.reduce((sum, l) => sum + Number(l.quantidade_atual || l.quantidade), 0) || 0;
            const baseQtd = Math.max(0, estoqueAntigo - lotesQtdOriginal);
            if (baseQtd > 0) {
                calcCustoTotal += baseQtd * custoAntigo;
                calcEstoqueTotal += baseQtd;
            }

            const custoMedio = calcEstoqueTotal > 0 ? calcCustoTotal / calcEstoqueTotal : custoNovo;
            ...
            // Se o lote que acabou de ser editado for de fato o mais recente do produto
            // Atualizamos as propriedades principais de exibição dele no grid pai e formulário
            if (loteMaisRecente && loteMaisRecente.id === loteEditId) {
                prodUpdatePayload.codigo = toUpperSafe(loteForm.codigo_barras || '');
                prodUpdatePayload.data_entrada = loteForm.data_entrada || new Date().toISOString().split('T')[0];
                prodUpdatePayload.data_validade = loteForm.data_validade || null;
            }

            const { error: prodError } = await supabase.from('produtos').update(prodUpdatePayload).eq('id', editing.id);
            
            if (prodError) {
                toast.error('Erro ao sincronizar estoque do produto com o lote.');
                return;
            }
            toast.success('Lote e Produto atualizados de forma sincronizada!');
        }
    } else {
  ...
  function handleEditarLoteConflito(lote: any, produto: Produto) {
      ...
      setShowForm(false); // Mantém principal fechado
      setShowLoteForm(true);
  }
  ...
  function handleEditarLoteHistorico(lote: any) {
    ...
    // Aqui nós NÃO fechamos o history modal, pois estamos sobrepondo a janela do lote
    // Ao salvar, o History vai refletir imediatamente devido à função load()
    setShowLoteForm(true);
  }
